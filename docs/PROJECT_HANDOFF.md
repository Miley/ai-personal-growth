...
最后更新：2026-07-30。后续有产品、架构或部署变化时，请优先更新本文件和 `DEPLOY_ALIYUN.md`。

## 产品目标

这是一个帮助个人持续成长的 iPhone PWA，而不是一个泛用聊天机器人。三个主题共用“低压力、可反复使用、反馈具体”的体验：

1. **表达能力**：每天按日期轮换一篇 2–3 分钟原创中文演讲范本；先理解结构，再用文字或录音复述。AI 反馈固定为总体判断、两个具体优点、一个最优先改进点、示范开头和下一次练习，避免一次给太多建议。
2. **另一个我**：私密但不黏人的 AI 同伴；支持文字、图片和语音输入，用于讲述经历、困惑与想法。
3. **英语**：以从易到难的阅读为主。每天首次打开时由云端生成一篇“今日新文章”，按本地日期和难度缓存，当天重复打开不再次调用；生成失败时明确显示本地备用文章，原有文章保留为更多练习。内容覆盖日常生活、旅行交流、阅读英语材料和陪 2 岁宝宝的日常输入；不把语音角色扮演作为主训练方式。

产品取舍：不强制每日打卡、不要求逐字背诵、不打分。表达训练同一天只保留最新一次复述记录；原始录音不保存，确认提交的文字和反馈只保存在本机浏览器。
...
4. 用函数计算的测试事件确认返回 200；涉及英语模块时检查 `daily-reading` 返回完整文章结构，再用 iPhone 实测一次。
